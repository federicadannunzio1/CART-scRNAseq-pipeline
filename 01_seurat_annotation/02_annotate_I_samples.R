...
out_doubt  <- paste0(base_dir, "Pipeline_I_prova/Doubt_resolution/") #change in _prova dir
out_umap   <- paste0(base_dir, "Pipeline_I_prova/Annotation_UMAP/")
out_mye    <- paste0(base_dir, "Pipeline_I_prova/Myeloid_search/")
  ...
  process_sample <- function(s_name, raw_obj, p) {
    ...
    obj <- subset(raw_obj,
                  subset = nFeature_RNA > 800 &
                           percent.mt < 7)
                           #azimuth_class != "dnT"). #commenting dnt deselection
...
# Helper: aggiunge label in grassetto con sfondo bianco ai cluster.
# DimPlot con label=TRUE usa geom_text senza controllo su fontface;
# LabelClusters() permette fontface="bold" e box=TRUE (sfondo).
add_bold_labels <- function(p, obj, id = "cell_type",
                             size = 4.5, box = TRUE) {
  LabelClusters(
    plot     = p,
    id       = id,
...
plot_umap_consistent <- function(obj, sample_name, out_path) {
  ...
  # Label=FALSE → poi aggiunte con LabelClusters (grassetto + box)
  make_dp <- function(show_label) {
    p <- DimPlot(obj, reduction = "umap", group.by = "cell_type",
      ...
      ggtitle(paste0(sample_name,
                     ifelse(show_label, " – Con label",
                            " – Senza label"))) +
      theme_classic(base_size = 12) +
      theme(plot.title = element_text(hjust = 0.5, face = "bold",
                                      size = 13),
            legend.position = "none")
    if (show_label) p <- add_bold_labels(p, obj)
    p
  }
...
#    proliferanti. Usata per Me_I C1/C3/C4 e Ca_I C0.
#
#    Logica (in ordine di priorità):
#      1. Se max(CD4, CD8) < 0.05 → segnale lineare troppo basso
#         per entrambi i marcatori (frequente in scRNA-seq per CD4).
#         Tiebreaker: score citotossico (GZMB/PRF1/NKG7).
#           - s_cytotox alto (>0.05) → "Proliferating CD8+ T cells"
#           - altrimenti → "Proliferating CD4+ T cells"
#           (in campioni I il prodotto è prevalentemente CD4;
#            nei campioni AB il contesto orienta la scelta)
#      2. CD8 > CD4 → "Proliferating CD8+ T cells"
#      3. CD4 > CD8 → "Proliferating CD4+ T cells"
#      4. CD4 == CD8 (raro) → tiebreaker citotossico come sopra
#
#    Non viene mai restituito "Proliferating T cells": il label
#    ambiguo è stato rimosso deliberatamente perché poco informativo.
#    Confidence = MEDIA quando max(CD4,CD8) < 0.05 (avvisa l'utente).
characterize_prolif <- function(obj, cluster_id) {
  ...
  if (!cl %in% colnames(avg)) {
    cat(sprintf("  [WARN] Cluster %s non trovato in avg_expr – uso tiebreaker citotossico.\n", cl))
    # Impossibile calcolare: default conservativo
  ...
  # Score citotossico come tiebreaker (media raw di GZMB/PRF1/NKG7)
  cytotox_genes <- c("GZMB","PRF1","NKG7","GNLY","GZMA")
  cytotox_genes <- cytotox_genes[cytotox_genes %in% rownames(obj)]
  avg_all <- fix_colnames(
    AverageExpression(obj, features = cytotox_genes,
                      group.by = "seurat_clusters",
                      assay = "RNA", slot = "data")$RNA)
  s_cytotox_cl <- if (length(cytotox_genes) > 0 && cl %in% colnames(avg_all))
    mean(avg_all[cytotox_genes, cl]) else 0

  cat(sprintf("  C%s | CD4=%.4f | CD8=%.4f | cytotox=%.4f",
              cl, avg_cd4, avg_cd8, s_cytotox_cl))

  label <- if (max(avg_cd4, avg_cd8) < 0.05) {
    # Entrambi molto bassi: tiebreaker citotossico
    if (s_cytotox_cl > 0.05) {
      cat(" → CD4/CD8 entrambi <0.05, citotox alto → Proliferating CD8+ T cells [MEDIA]\n")
      "Proliferating CD8+ T cells"
    } else {
      cat(" → CD4/CD8 entrambi <0.05, citotox basso → Proliferating CD4+ T cells [MEDIA]\n")
      "Proliferating CD4+ T cells"
    }
  } else if (avg_cd8 > avg_cd4) {
    ...
    # Esattamente uguali (raro): tiebreaker citotossico
    if (s_cytotox_cl > 0.05) {
      cat(" → CD4==CD8, citotox alto → Proliferating CD8+ T cells [MEDIA]\n")
      "Proliferating CD8+ T cells"
    } else {
      cat(" → CD4==CD8, citotox basso → Proliferating CD4+ T cells [MEDIA]\n")
      "Proliferating CD4+ T cells"
    }
  }
  label
...
make_dp_mini <- function(obj, nm) {
  ...
  p <- DimPlot(obj, reduction = "umap", group.by = "cell_type",
    ...
    theme(plot.title = element_text(hjust = 0.5, face = "bold"),
          legend.position = "none")
  add_bold_labels(p, obj, size = 3.5)
}
