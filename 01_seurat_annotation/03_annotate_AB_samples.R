...
auto_annotate <- function(obj, sample_name) {
  ...
  for (cl in clusters_chr) {
    ...
    # ── 16. PROLIFERATING ────────────────────────────────────
    # Logica CD4/CD8 (mai "Proliferating T cells" generico):
    #   - Se max(CD4, CD8) < 0.05 → entrambi bassissimi in
    #     scRNA-seq (CD4 sottoespresso post-attivazione).
    #     Tiebreaker: s_cytotox > 0.05 → CD8, altrimenti CD4.
    #   - CD8 > CD4 → Proliferating CD8+
    #   - CD4 > CD8 → Proliferating CD4+
    #   - CD4 == CD8 (raro) → tiebreaker citotossico
    # Confidence = MEDIA se max(CD4,CD8) < 0.05.
    } else if (s_mk67 > 0.5 || s_prolif > 0.05) {
      if (max(s_cd4, s_cd8) < 0.05) {
        # Entrambi i marcatori lineage molto bassi
        if (s_cytotox > 0.05) {
          label     <- "Proliferating CD8+ T cells"
          rationale <- sprintf("MKI67=%.3f, CD4/CD8 entrambi<0.05, cytotox=%.3f→CD8",
                               s_mk67, s_cytotox)
        } else {
          label     <- "Proliferating CD4+ T cells"
          rationale <- sprintf("MKI67=%.3f, CD4/CD8 entrambi<0.05, cytotox=%.3f→CD4",
                               s_mk67, s_cytotox)
        }
        confidence <- "MEDIA"
      } else if (s_cd8 > s_cd4) {
        label     <- "Proliferating CD8+ T cells"
        rationale <- sprintf("MKI67=%.3f, prolif=%.3f, CD8=%.3f>CD4=%.3f",
                             s_mk67, s_prolif, s_cd8, s_cd4)
      } else if (s_cd4 > s_cd8) {
        label     <- "Proliferating CD4+ T cells"
        rationale <- sprintf("MKI67=%.3f, prolif=%.3f, CD4=%.3f>CD8=%.3f",
                             s_mk67, s_prolif, s_cd4, s_cd8)
      } else {
        # CD4 == CD8 esattamente (raro): tiebreaker citotossico
        if (s_cytotox > 0.05) {
          label     <- "Proliferating CD8+ T cells"
          rationale <- sprintf("MKI67=%.3f, CD4==CD8=%.3f, cytotox=%.3f→CD8",
                               s_mk67, s_cd4, s_cytotox)
        } else {
          label     <- "Proliferating CD4+ T cells"
          rationale <- sprintf("MKI67=%.3f, CD4==CD8=%.3f, cytotox=%.3f→CD4",
                               s_mk67, s_cd4, s_cytotox)
        }
        confidence <- "MEDIA"
      }
